...
class Lianjiahouse2Pipeline:
    ...
    def from_crawler(cls, crawler):
        return cls(
            ...
            mongo_db=crawler.settings.get('MONGO_DATABASE', 'lianjia4')
        )
...
class LianjiaImagePipeline(ImagesPipeline):
    def get_media_requests(self, item, info):
        for image_url in item['images_urls']:
            # 将图片地址传入Request,进行下载，同时将item作为参数添加到Request中
            yield Request(image_url, meta={'item': item})

    def file_path(self, request, response=None, info=None, *, item=None):
        # 从Request中获取item，以及房屋标题做文件夹名称
        item = request.meta['item']
        image_folder = item['house_name']
        # 使用图片url做图片存储名称
        image_guild = request.url.spilt('/')[-1]
        # 图片保存，文件夹/图片
